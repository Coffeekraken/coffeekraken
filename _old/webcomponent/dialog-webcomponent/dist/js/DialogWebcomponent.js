...
var _sendForm = _interopRequireDefault(require("@coffeekraken/sugar/js/dom/sendForm"));
 ...
 */
class DialogWebcomponent extends _SWebComponent.default {
  /**
  ...
  componentMount() {
    super.componentMount(); // variables

    this._cachedContent = null;
    this._html = null;
    this._allowModalClose = false; // get the triggerer
    ...
    if (this._triggerer.tagName.toLowerCase() === 'form') {
      this._triggerer.addEventListener('submit', e => {
        ...
        if (!e.target.checkValidity()) return; // prevent default form behavior

        e.preventDefault(); // send form

        (0, _sendForm.default)(e.target).then(response => {
          // remove the cached content
          this._cachedContent = null; // open the dialog

          this.open((0, _strToHtml.default)(response));
        });
      });
    } else {
      // handle openOn
  ...
  open(content = null) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._reject = reject; // get content from passed parameter or settings

      if (!content) {
        if (this.props.content) content = this.props.content;else if (this._triggerer) content = this._triggerer.getAttribute('href');
      ...
      if (!this._cachedContent) {
        ...
        if (!content) {
          ...
          this._cachedContent = content.innerHTML; // open
          ...
          const $dom = document.querySelector(content); // check if the $dom element is a template or not
          ...
          this._cachedContent = iframe; // open
          ...
          ajx.send().then(response => {
            // set the content
            this._cachedContent = response; // open
  ...
  _open() {
    // add the body class
    document.body.classList.add(`${this._componentNameDash}--opened`); // open counter

    DialogWebcomponent.counter++; // create the DOM structure

    if (!this._template) {
      this._html = (0, _strToHtml.default)(`
      ...
      if (!this.props.modal) {
        (0, _style.default)(this.refs.overlay, {
          cursor: 'pointer'
        });
    ...
    if (typeof this._cachedContent === 'string') {
      ...
      if (this._cachedContent.parentNode && !this._domRestorePlaceholder) {
        ...
        (0, _insertAfter.default)(this._domRestorePlaceholder, this._cachedContent);
  ...
  _onKeyup(e) {
    e.preventDefault(); // check if is escape key
  ...
  close(force = true) {
    // check if is a modal
    if (this.props.modal && !this._allowModalClose && !force) return; // close
  ...
  _close() {
    ...
    this.addComponentClass(this.refs.elm, null, null, 'out'); // let enough time to apply the new animations
    // in order to get right values from the getAnimationProperties

    this.mutate(() => {
      let longestAnimationTime = 0; // longest animation

      [this.refs.content, this.refs.overlay, this.refs.elm].forEach(elm => {
        // get animation properties
        const animationProperties = (0, _getAnimationProperties.default)(elm); // check if totalDuration is longest that the previous one
      ...
      document.removeEventListener('keyup', this._onKeyup); // wait end animation to remove the dialog

      setTimeout(() => {
        // restore the place of the content if is a placeholder
        if (this._domRestorePlaceholder && this._cachedContent.nodeName) {
          (0, _insertAfter.default)(this._cachedContent, this._domRestorePlaceholder);
        ...
        this.removeComponentClass(this.refs.elm, null, null, 'out'); // remove the container from the dom
  ...
  ok(value = null) {
    if (!this.isOpened()) return; // resolve the promise if exist

    try {
      if (this._resolve && value) {
        this._resolve(value);
      }
    } catch (e) {} // events and callback


    this.dispatchComponentEvent('ok', value);
    this.props.onOk && this.props.onOk(value); // close the dialog
  ...
  dismiss(value = null) {
    ...
    this.props.onDismiss && this.props.onDismiss(value); // close the dialog
...
exports.default = DialogWebcomponent;

_defineProperty(DialogWebcomponent, "counter", 0);

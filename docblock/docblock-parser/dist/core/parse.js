...
var _processExample = require('../utils/processExample');

var _processExample2 = _interopRequireDefault(_processExample);
...
function parse(stringToParse) {
	...
	try {
		...
		var lines = [];
		var _language = language || this._config.language || 'js';
		...
		lines.forEach(function (line) {
					...
					if (currentTagValue.length) {
						if (_typeof(data[currentTag]) === 'object') {
							if (currentTag === 'example') {
								// process example
								data[currentTag].body = (0, _processExample2.default)(currentTagValue.join("\n"));
							} else {
			...
			if (line.substr(0, 1) === '@') {
				...
				// get the tag name
				var name = splits[0].trim().substr(1);
				// unshift the name of the splits
				splits.shift();
				// process the line
				if (_this._config.tags['@' + name]) {
					_this._config.tags['@' + name](name, splits, data, _language);
				} else {

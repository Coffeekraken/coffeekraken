...
module.exports = async () => {
  ...
  __log(`Generating the Squid css file(s)...`, 'info');
  ...
  const sourceFiles = __glob.sync(__path.resolve(__dirname, '../../') + '/src/css/**/*.bundle.scss');

  sourceFiles.forEach(async (srcFile) => {

    __log(`Compiling the "${srcFile.replace(__path.resolve(__dirname, '../../'), '')}" file...`, 'info');
    ...
    __sass.render({
      ...
      importer: __globImporter()
        ...
        if(!error){
          ...
          __log('Writing the output raw Squid css file...', 'info');
          ...
          __log('Processing the output Squid css using PostCSS and some plugins (precss, autoprefixer, postcss-preset-env, cssnext, cssnano and rucksack-css)...', 'info');
          ...
          __postcss([
            ...
            __log('Writing the output processed Squid css file...', 'info');

...
// require('jquery');
function goldenLayout(node) {
  const interactiveDemoElm = node.querySelector('ck-interactive-demo');
  ...
  if (savedState !== null) {
    ...
    if (state._layout === layout) {
      myLayout = new _goldenLayout.default(state, node.querySelector('ck-interactive-demo'));
    } else {
      myLayout = new _goldenLayout.default(config, node.querySelector('ck-interactive-demo'));
    }
  } else {
    myLayout = new _goldenLayout.default(config, node.querySelector('ck-interactive-demo'));
  }

  myLayout.registerComponent('preview', function (container, componentState) {
    container.getElement().get(0).appendChild(interactiveDemoElm.querySelector('[ck-interactive-demo-preview]'));
  });
  [].forEach.call(document.querySelectorAll('template'), editor => {
    if (!editor.id) return;
    myLayout.registerComponent(editor.id, function (container, componentState) {
      container.getElement().append(document.importNode(editor.content, true));
    });
  }); // init layout

  myLayout.init(); // save state

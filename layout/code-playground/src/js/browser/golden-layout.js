...
export default function goldenLayout(node) {
	...
	if( savedState !== null) {
		...
		if (state._layout === layout) {
	    	myLayout = new GoldenLayout(state, node.querySelector('ck-interactive-demo'));
		} else {
			myLayout = new GoldenLayout( config, node.querySelector('ck-interactive-demo') );
		}
	} else {
	    myLayout = new GoldenLayout( config, node.querySelector('ck-interactive-demo') );
	}

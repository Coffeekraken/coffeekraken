    ...
    implements ISherlockAdapter
{
    ...
    async clients(cb: Function): Promise<void> {
        // current records
        const records = await this._pocketbase.collection('clients').getFullList({
            sort: '-name',
        });
    ...
    async service(serviceUid: string, cb: Function): Promise<void> {
        ...
        // realtime
        this._pocketbase.collection('services').subscribe(record.id, function (e) {
            if (e.action !== 'delete') {
                cb({
                    uid: e.record.uid,
                    name: e.record.name,
    ...
    async clientServices(clientUid: string, cb: Function): Promise<void> {
        const records = await this._pocketbase.collection('services').getFullList({
            $autoCancel: false,

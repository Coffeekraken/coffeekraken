...
export default function createProject(_recipe?: string) {
    return new __SPromise(
        async ({ resolve, reject, emit, pipe }) => {
            ...
            emit('log', {
                value: `- Installing dependencies using ${
                    isYarn ? 'yarn' : 'npm'
                }...`,
            });
            ...
            if (npmInitRes.stderr) {
                ...
                if (errorStr !== '' && !errorStr.match(/^warning\s/)) {
                    emit('log', {
                        value: `<red>[${
                            isYarn ? 'yarn' : 'npm'
                        }] Error during the dependencies instalation. More details in the sugar.log file at the root of your project</red>`,
                    });
                    emit('writeLog', {

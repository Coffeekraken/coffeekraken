<template>
    ...
    >
        <div
            :class="_classStringToObject(_component?.className('_root'))"
            ref="$root"
        >
            <div
                :class="
            ...
            >
                <div
                    ref="$slides"
                    :class="
                        _classStringToObject(_component?.className('__slides'))
                    "
                >
                    <slot />
                </div>
            ...
            <template v-if="_slideElements.length && nav">
                <div class="s-slider__nav">
                    <template
                        :key="index"
                        v-for="(child, index) in _slideElements"
                    >
                        <div
                            :class="
                                _classStringToObject(
...
export default {
    ...
    mounted() {
        ...
        this._id = this.id ?? `s-slider-${__uniqid()}`;
    ...
    methods: {
                    ...
                    if (!$slidesWrapper._isBehaviorInited) {
                        ...
                        $slidesWrapper.addEventListener('scroll', (e) => {
                            ...
                            scrollTimeout = setTimeout(() => {
                                ...
                                this.goTo(
                                    this.getSlideIdByElement($inViewSlide),
                                );
                            }, 100);
                        });
                    }
        ...
        goTo(slideId) {
            // call the behavior
            const behaviorFn =
                this.behaviors?.[this.behavior] ??
                this._behaviors[this.behavior];
            behaviorFn?.({

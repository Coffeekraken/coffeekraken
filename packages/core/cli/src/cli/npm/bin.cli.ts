    ...
    if (!argsObj.package) {
        ...
        for (let i = 0; i < Object.keys(packagesObj).length; i++) {
            ...
            if (json.name === argsObj.package) {
                packageJson = json;
                packageJson.absolutePath = _path.resolve(
                    process.cwd(),
                    Object.keys(packagesObj)[i],
                );
                break;
            }
        }
        ...
        Object.keys(packageJson.bin).forEach((binName) => {
            ...
            const binAbsolutePath = _path.resolve(
            ...
            switch (argsObj.action) {
                ...
                case 'install':
                    ...
                    console.log(
                        `The "<yellow>${binName}</yellow>" bin from the package "<cyan>${
                            packageJson.name
                        }</cyan>" has been successfully installed ${
                            argsObj.global ? '<magenta>globaly</magenta>' : ''
                        }`,
                    );
                ...
                case 'uninstall':
                    console.log(
                        `The "<yellow>${binName}</yellow>" bin from the package "<cyan>${
                            packageJson.name
                        }</cyan>" has been successfully uninstalled ${
                            argsObj.global ? '<magenta>globaly</magenta>' : ''
                        }`,
                    );

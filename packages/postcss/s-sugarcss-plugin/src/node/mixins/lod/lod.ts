...
export interface SSugarcssPluginLodMixinParams {
    level: number | string;
    method: 'file' | 'class';
...
export default function ({
    ...
    params: Partial<SSugarcssPluginLodMixinParams>;
    atRule: any;
    settings: any;
    postcssApi: any;
}) {
    const finalParams = <SSugarcssPluginLodMixinParams>{
        level: 0,
    ...
    if (atRule.parent.type === 'root') {
        atRule.nodes.forEach((node) => {
            if (!node.selector) {
                throw new Error(
                    `<red>[SSugarcssPlugin.lod]</red> When using the "<yellow>@s.lod</yellow>" mixin at root level, children MUST be css rule and not a property or anything else...`,
                );
            }
        });
    }

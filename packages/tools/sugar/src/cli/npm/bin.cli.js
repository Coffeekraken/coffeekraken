...
module.exports = async function bin(stringArgs = '') {
  ...
  if (!argsObj.package) {
    ...
    if (!packageJson) throw `Sorry but no package has been found with the name "<yellow>${argsObj.package}</yellow>"...`;
    // check for bins
    if (!packageJson.bin) throw `Sorry but the package named "<yellow>${packageJson.name}</yellow>" does not have any bin's to install...`;
    Object.keys(packageJson.bin).forEach(binName => {
      ...
      switch(argsObj.action) {
        case 'i':
        case 'install':
          const symlinkCommand = `ln -s --no-target-directory ${_path.relative(binFolderPath, binAbsolutePath)} ${binName}`;
          if (!_fs.existsSync(`${binFolderPath}/${binName}`)) {

...
export default class SDocblock {
    ...
    constructor(source, settings = {}) {
        ...
        if (__isPath(source)) {
            if (!__isNode())
                throw new __SError(`Sorry but in a none node environement the SDocblock class can take only a String to parse and not a file path like "<yellow>${source}</yellow>"...`);
            const __fs = require('fs');
            if (!__fs.existsSync(source))
                throw new __SError(`Sorry but the passed source path "<yellow>${source}</yellow>" does not exists on the filesystem...`);

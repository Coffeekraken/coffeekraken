...
module.exports = class SSugarAppTerminalUi extends __SBlessedComponent {
    ...
    _updateBottomBar() {
        ...
        Object.keys(this._initialParams.modules).forEach((moduleName, i) => {
            ...
            const moduleString = ` ${spinner}${moduleObj.id} `
                ...
                .split('')
                .map((char) => {
                return `<bg${__upperFirst(bg)}><${fg}>${char}</${fg}></bg${__upperFirst(bg)}>`;
            })
                .join('');
    ...
    _moduleError(data, metas) {
        ...
        if (msg.length > 36)
            msg = msg.slice(0, 33) + '...';
        const $errorNotification = new __SNotification(data.module.name || data.module.id, msg, {
            bg: 'red',
            onClick: () => {
                this._showModule(moduleObj.id);
            }
    ...
    _moduleSuccess(data, metas) {
        ...
        if (msg.length > 36)
            msg = msg.slice(0, 33) + '...';
        const $successNotification = new __SNotification(data.module.name || data.module.id, msg, {
            bg: 'green',
            onClick: () => {
                this._showModule(moduleObj.id);
            }
    ...
    _moduleStart(data, metas) {
        ...
        if (msg.length > 36)
            msg = msg.slice(0, 33) + '...';
        const $startNotification = new __SNotification(data.module.name || data.module.id, msg, {
            bg: 'yellow',
            onClick: () => {
                this._showModule(moduleObj.id);
            }
    ...
    _initWelcome(initialParams) {
        ...
        const byLineSpaces = Math.round((__countLine(projectLine) - __countLine(byLine)) / 2) - 1;
        const projectLines = [
    ...
    _initConsoles() {
        ...
        Object.keys(this._modules).forEach((moduleName) => {
            ...
            const $topBar = __blessed.box({
                ...
                content: __parseHtml(`<yellow>${moduleObj.name}</yellow> | <white>${moduleObj.id}</white>`)
            });
            let OutputClass;
            if (moduleObj.ui) {

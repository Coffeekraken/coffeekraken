...
class SNav {
     ...
     */
    getItem(from) {
        if (from instanceof SNavItem_1.default)
            return from;
    ...
    removeItem(...items) {
        items.forEach((item) => {
            if (item instanceof SNavItem_1.default) {
                ...
                if (item > this._itemsArray.length - 1)
                    return;
     ...
     */
    toMarkdown(settings = {}) {
        settings = deepMerge_1.default(this._settings.markdown, settings);
        const itemsArray = [];
        this._itemsArray.forEach((sNavItem) => {
            if (sNavItem instanceof SNavItem_1.default) {
                itemsArray.push('\t'.repeat(settings.indent) + sNavItem.toMarkdown(settings));
            }
            else if (sNavItem instanceof SNav) {
                itemsArray.push(sNavItem.toMarkdown({
                    ...settings,
     ...
     */
    toHtml(settings = {}) {
        settings = deepMerge_1.default(this._settings.html, settings);
        const itemsArray = [
            settings.ordered
...
${'\t'.repeat(settings.indent)}</li>`);
        ...
        return pretty_1.default(itemsArray.join('\n'));
    }
}

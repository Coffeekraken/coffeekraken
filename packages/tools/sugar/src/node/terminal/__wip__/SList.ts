...
module.exports = class TerminalList {
    ...
    __terminalKit.on('key', (name, matches, data) => {
      ...
      switch (eventName) {
        ...
        case 'DOWN':
          ...
          break;
        case 'RIGHT':
          if (this._columnsCount <= 1) return;
        ...
        case 'ENTER':
          ...
          const selectedObj = {
            ...
            formatted: this._settings.selected(
              this._items[selectedItemIdx],
              selectedItemIdx
            )
          };
          if (this._settings._onSelect) this._settings._onSelect(selectedObj);
  ...
  get(settings = {}) {
    ...
    if (settings.columns) this._columnsCount = settings.columns;
    ...
    if (typeof maxWidth === 'string' && maxWidth.slice(-1) === '%') {
      maxWidth = Math.round(
        (this._maxWidth / 100) * parseInt(this._settings.width)
      );
    ...
    if (!items.length) return settings.format === 'string' ? '' : [];

    let columns = [];
    let splitEvery = Math.round(items.length / settings.columns);
    if (settings.columns > 1 && splitEvery > 0) {
      columns = __splitEvery(
        items,
        Math.round(items.length / settings.columns)
      );
    } else {
      columns.push(items);
    }
      ...
      columns.forEach((columnItems, columnIdx) => {
        ...
        if (columnItems[currentLineIdx]) {
          if (
            ...
            itemString = this._settings.selected(
              columnItems[currentLineIdx],
              (columnIdx + 1) * (rowIdx + 1)
            );
          } else {
            itemString = this._settings.item(
              columnItems[currentLineIdx],
              (columnIdx + 1) * (rowIdx + 1)
            );
          }

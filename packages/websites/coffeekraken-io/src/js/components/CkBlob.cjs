...
var CkBlob_exports = {};
__export(CkBlob_exports, {
  default: () => CKBlob,
  define: () => define
});
module.exports = __toCommonJS(CkBlob_exports);
...
var import_OBJLoader = require("./lib/three/examples/jsm/loaders/OBJLoader.js");
var import_RGBELoader = require("./lib/three/examples/jsm/loaders/RGBELoader");
var import_EffectComposer = require("./lib/three/examples/jsm/postprocessing/EffectComposer.js");
var import_RenderPass = require("./lib/three/examples/jsm/postprocessing/RenderPass.js");
var _a;
class CKBlob extends import_s_lit_component.default {
  constructor() {
    super({
      litComponent: {
        shadowDom: false
    ...
    this._isDark = (_a = document.querySelector("html").getAttribute("theme")) == null ? void 0 : _a.toString().includes("dark");
  ...
  async firstUpdated() {
    ...
    if (this._isDark) {
      ...
      backLight = new THREE.PointLight(16777215, 1);
      backLight.position.set(8, 8, -10);
    }
    ...
    var ambientLight;
    if (this._isDark) {
      ambientLight = new THREE.AmbientLight(16777215, 0.8);
    } else {
      ambientLight = new THREE.AmbientLight(16777215, 0.9);
    }
    const helper = new THREE.CameraHelper(light.shadow.camera);
    this._planePoints = this.createPlanePoints(2172712);
    this._planePointsLight = this.createPlanePoints(2172712, 1, "6.0");
  ...
  _updatePlaneWave(object) {
    ...
    function wavesBuffer(waveSize, magnitude) {
      const theTime = performance.now() * 1e-3;
  ...
  createPlanePoints(color, alphaVariation, pointSize = "8.0") {
    ...
    var shaderMaterial = new THREE.ShaderMaterial({
      uniforms: {
        color: { value: new THREE.Color(this._isDark ? color : 15198183) }
      },
      vertexShader,
      fragmentShader,
      transparent: true
  ...
  createGrainMaterial(texturePath) {
    return new Promise(async (resolve) => {
      ...
      const bumpMap = await this.loadTexture(`${import_s_sugar_config.default.get("serve.img.url")}/3d/coffeeGrain/coffeeGrainBumpMap.jpg`);

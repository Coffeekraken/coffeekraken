...
module.exports = class Script {
  ...
  _resetStack() {
    // reset the stack
    this._stack = [
      `press ${chalk.yellow.bold("r")} to run or ${chalk.red.bold(
        "e"
      )} to exit the ${chalk.bold(this.id)} script`
    ];
  }
  ...
  run() {
    ...
    // reset the stack
    this._stack = [];
    // save the timestamp of the start
    const startTime = new Date().getTime();
    // append that the script has been launched
    this._stack.push(
      chalk.yellow(`${logSymbols.warning} Start ${chalk.bold(this.id)}\n`)
    );
    this._stack.push(
      chalk.yellow(`----------------------------------------\n`)
    );
    ...
    this._childScript.on("exit", (code, signal) => {
      ...
      if (code === 0) {
        ...
        this._stack.push(
          chalk.green(
            `${logSymbols.success} Completed ${chalk.bold(
              this.id
        ...
        this._stack.push(
          chalk.red(
            `${logSymbols.error} Error in ${chalk.bold(
              this.id
            )} after ${(endTime - startTime) / 1000}s\n`
          )
        );
      }
      // dispatch new data
  ...
  watch() {
    ...
    this._watcher = chokidar.watch(
      watchObj.paths,
      Object.assign({}, watchObj.options)
    );
    this._watcher
      .on("add", path => {
        this.run();

...
module.exports = function hotkey(key, settings) {
    ...
    process.stdin.on('keypress', function (ch, keyObj) {
      ...
      Object.keys(hotkeyStack).forEach(id => {
        ...
        obj.key.toString().split(',').map(m => m.trim()).forEach(key => {
          ...
          if (keyObj.ctrl) pressedKey = `ctrl${obj.settings.splitKey}${pressedKey}`;
          if (keyObj.shift) pressedKey = `shift${obj.settings.splitKey}${pressedKey}`;
          if (keyObj.meta) pressedKey = `alt${obj.settings.splitKey}${pressedKey}`;
  ...
  const promise = new __SPromise((resolve, reject, trigger, cancel) => {}, {}).on('key,press', key => {
    if (settings.once) {
      promise.cancel();
    }
  }).on('finally,cancel', () => {

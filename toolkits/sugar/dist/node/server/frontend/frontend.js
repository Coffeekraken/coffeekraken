...
module.exports = function (args) {
  ...
  if (settings.menu) {
    ...
    Object.keys(settings.menu).forEach(async menuName => {
      ...
      menuStack[menuName] = await generatorObj.fn(generatorObj.directory); // add the nav to the main navigation
  ...
  server.get('/', async (req, res) => {
    ...
    if (__fs.existsSync(indexViewPath)) {
      ...
      const viewContent = __fs.readFileSync(`${__sugarConfig('views.rootDir')}/index.blade.php`, 'utf8');

      let view = 'index';

      const tmpDir = __path.resolve(__sugarConfig('views.rootDir'), 'tmp'); // check if the view does extend a special layout
      ...
      if (!content.includes('<body')) {
        const baseContent = __fs.readFileSync(__dirname + '/static/index.html', 'utf8');

        const stringToCompile = baseContent.replace('[content]', content); // const result = renderTemplate(stringToCompile, templateData);
  ...
  Object.keys(settings.handlers).forEach(async pageName => {
    ...
    server.get([`${handlerSettings.slug}/*`, `${handlerSettings.slug}`], async (req, res) => {
      try {
        const handlerPromise = handlerFn(req, server);

        __SPromise.pipe(handlerPromise, promise);

        const response = await handlerPromise; // handle response

        const view = response.view || 'pages.404';
        const data = response.data || null;
        const title = response.title || 'Page not found';
        const type = response.type || 'text/html'; // prepariong the result

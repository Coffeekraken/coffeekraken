...
function alreadyLoaded(link) {
  ...
  for (var i = 0; i < document.styleSheets.length; i++) {
    if (document.styleSheets[i].href && document.styleSheets[i].href.match(href)) {
      ...
      result = true;
    } else if (i == document.styleSheets.length - 1) {// Fallback. There is no request for the css file.
    }
  }
...
function linkLoaded(link, callback = null) {
  return new Promise((resolve, reject) => {
    ...
    if (alreadyLoaded(link)) {
      // resolve promise
      resolve(link); // call the callback if exist
      ...
      img.addEventListener('error', e => {
        ...
        // resolve the promise
        resolve(link); // callback if exist
      ...
      img.src = link.href; // document.body.appendChild(img);
    }
  });
}

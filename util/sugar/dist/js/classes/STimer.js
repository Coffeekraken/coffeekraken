...
class STimer {
  ...
  constructor(duration, settings = {}) {
    ...
    _defineProperty(this, "_ticks", []);

    _defineProperty(this, "_completes", []);
    ...
    this._duration = duration; // updating settings
  ...
  reset(start = false) {
    ...
    if (start) this.start(); // maintain chainability
  ...
  pause() {
    ...
    clearTimeout(this._tickSetTimeout); // maintain chainability
  ...
  stop() {
    // reset
    this.reset(); // maintain chainability

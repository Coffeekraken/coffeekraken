...
var _constructorName = _interopRequireDefault(require("../utils/objects/constructorName"));
...
class SWatcher {
  ...
  _defineProp(obj, property, value, objPath, descriptor = null) {
    ...
    if (this._watchStack[objPath]) return; // store the current value
    ...
    _set(value); // make sure we have the good currentDescriptor
    ...
    Object.defineProperty(obj, property, {
      ...
      enumarable: descriptor && descriptor.enumarable !== undefined ? descriptor.enumarable : currentDescriptor && currentDescriptor.enumarable !== undefined ? currentDescriptor.enumarable : true // writable : currentDescriptor && currentDescriptor.writable !== undefined ? currentDescriptor.writable : true
  ...
  _overrideArrayMethod(array, methods, objPath, setValueCb) {
    const _this = this; // grab the old value


    const oldVal = array.slice(0); // loop on each methods to override
    ...
    methods.forEach(method => {
      array[method] = function () {
        ...
        const ret = Array.prototype[method].apply(this, arguments); // set value callback

        setValueCb(this); // _notify

        _this._notify(objPath, this, oldVal, updateInfo); // return the new value
  ...
  _watch(object, path, cb, descriptor = null) {
    ...
    if (split.length > 1) {
      property = split.pop();
      obj = (0, _get2.default)(object, split.join("."));
    } else {
      property = split[0];
    }

    let currentValue = null;
    currentValue = (0, _get2.default)(object, path); // if is undefined, create the value at null

    if (obj === undefined || currentValue === undefined) {
      (0, _set2.default)(obj, path, null); // _set(this, split.join('.'),null);
      // throw `It's not possible to watch the property ${path} cause it does not exist...`;
    } // define the property proxy


    this._defineProp(obj, property, currentValue, path, descriptor); // register new watch

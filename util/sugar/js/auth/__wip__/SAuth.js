...
module.exports = (_temp = /*#__PURE__*/function () {
  ...
  _createClass(SAuth, [{
    ...
    value: async function authInfo(settings = {}) {
      ...
      const authInfo = await this.ask(settings); // return the getted infos
    ...
    key: "inject",
    value: function inject(injector, requestConfig, authInfo = this._authInfo) {
      ...
      let finalRequestConfig = {}; // check if the injector is a string
    ...
    value: async function ask(settings = {}) {
      ...
      const rawInfos = await this._adapter.ask(settings); // format the auth infos using the corresponsing formater
      ...
      const cryptedInfos = __cryptObject.encrypt(formatedInfos, __machineIdSync()); // the infos are ok so we save them in cache


      await this._settings.cache.set(settings.type, cryptedInfos, {}); // save the auth info into memory

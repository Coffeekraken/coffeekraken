  ...
  for (let i = 0; i < Object.keys(definitionObj); i++) {
    ...
    if (value !== undefined && argDefinition.type) {
      if (argDefinition.type.toLowerCase() === 'array') {
        if (!Array.isArray(value)) return `The property "${argName}" has to be an Array. You've passed a "${typeof value}"...`;
      } else if (typeof value !== argDefinition.type.toLowerCase()) {
        return `The property "${argName}" has to be of type "${argDefinition.type}". You've passed a "${typeof value}"...`;
      }
    } // check required

...
var _asyncForEach = _interopRequireDefault(require("../array/asyncForEach"));
...
class SPromise {
  ...
  constructor(executor, settings = {}) {
    ...
    this._executorFn = executor; // extend settings

    this._settings = (0, _deepMerge.default)({
      stacks: null
    }, settings); // init the master promise returned
  ...
  async _reject(arg, stacksOrder = ['catch', 'rejected', 'finally']) {
    ...
    this._destroy(); // return the stack result
  ...
  async _triggerStack(stack, initialValue) {
    ...
    if (typeof stack === 'string') stack = this._stacks[stack]; // filter the catchStack
    ...
    for (let i = 0; i < stack.length; i++) {
      ...
      let callbackResult = item.callback.apply(this, [currentCallbackReturnedValue, this]); // check if the callback result is a promise
  ...
  on(stacks, callback) {
    ...
    stacks.forEach(name => {
      ...
      if (!this._masterPromise[name]) return; // calling the registration method

      this._masterPromise[name](callNumber, callback);
    }); // maintain chainability

...
import __SAjaxRequest from "./SAjaxRequest";
...
export default class SAjax {
  ...
  constructor(request, settings = {}) {
    ...
    if (!(request instanceof __SAjaxRequest)) {
      this._requestSettings = new __SAjaxRequest(request);
    } else {
  ...
  _onSuccess(response) {
    ...
    const hash = requestSettings.url.indexOf('#') !== -1 ? requestSettings.url.split('#')[1] : false;
  ...
  _send() {
    ...
    if (this._settings.beforeSend) {
      requestSettings = this._settings.beforeSend(
        requestSettings,
        this._requestsCount
      );
    }
    ...
    __axios(requestSettings).then(this._onSuccess).catch(this._onError);

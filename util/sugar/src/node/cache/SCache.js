...
const __SCacheAdapter = require('./adapters/SCacheAdapter');
...
module.exports = class SCache {
  ...
  constructor(name, settings = {}) {
    ...
    this._settings = __deepMerge({
      ttl: -1,
      deleteOnExpire: true,
      adapter: new (require('./adapters/SCacheFsAdapter'))(),
      parse: JSON.parse,
  ...
  get adapter() {
    ...
    if (typeof adapter === 'string' && this._defaultAdaptersPaths[adapter]) {
      this._adapter = new require(this._defaultAdaptersPaths[adapter])();
  ...
  async get(name, valueOnly = true) {
    ...
    const rawValue = await this.adapter.get(`${this._name}.${name}`);
    // check that we have a value back
    if (!rawValue || typeof rawValue !== 'string') return null;
    // parse the raw value back to an object
    const value = this.adapter.parse ? this.adapter.parse(rawValue) : this._parse(rawValue);
    // check if the item is too old...
    if (value.deleteAt !== -1 && value.deleteAt < new Date().getTime()) {
      // this item has to be deleted
      if (value.deleteOnExpire) await this.adapter.delete(name);
  ...
  async set(name, value, settings = {}) {
    ...
    const deleteAt = finalSettings.ttl === -1 ? -1 : new Date().getTime() + __convert(typeof finalSettings.ttl === 'number' ? `${finalSettings.ttl}s` : finalSettings.ttl, 'ms');
    const valueToSave = {
      name,
      value,
    ...
    const stringifiedValueToSave = this.adapter.stringify ? this.adapter.stringify(valueToSave) : this._stringify(valueToSave);

    // use the adapter to save the value
    return this.adapter.set(`${this._name}.${name}`, stringifiedValueToSave);
  }
   ...
   */
  async delete(name) {
    return this.adapter.delete(`${this._name}.${name}`);
  }
   ...
   */
  async clear() {
    return this.adapter.clear(this._name);
  }

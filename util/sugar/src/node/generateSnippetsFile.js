...
(() => {
  ...
  __find.find('@namespace', __dirname, '.js$').then(files => {
    ...
    for (let i = 0; i < Object.keys(files).length; i++) {
      ...
      // "Sugar.is.node": {
      //   "scope": "javascript,node,nodejs",
      //   "prefix": "Sugar.is.node",
      //   "body": "Sugar.is.node($1);",
      //   "description": "Something node sugar cool!!!"
      // }
    ...
    if (rootPath) {
      try {
        __writeJsonSync(rootPath + '/.vscode/sugar.node.code-snippets', snippetsObj);
      } catch (e) {
        console.log(e);
      }
    }

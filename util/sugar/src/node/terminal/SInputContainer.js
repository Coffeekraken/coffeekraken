...
module.exports = class SInput extends __blessed.box {
    ...
    __terminalKit.on('key', (name, matches, data) => {
      ...
      switch (eventName) {
        case 'ENTER':
          break;
        case 'BACKSPACE':
          if (this._value.length) this._value = this._value.slice(0, -1);
          this._events.emit('value', this._value);
          break;
        default:
          if (data.isCharacter) this._value += name;
          this._events.emit('value', this._value);
          break;
      }
    });
  ...
  get(settings = {}) {
    ...
    if (this._settings.message) {
      inputLine += this._settings.messageFormater ? this._settings.messageFormater(this._settings.message + ': ') : this._settings.message + ': ';
    }
    if (this._value.length) {
      inputLine += this._settings.valueFormater ? this._settings.valueFormater(this._value) : this._value;
    }

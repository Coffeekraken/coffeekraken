...
module.exports = class TerminalList {
    ...
    __terminalKit.on('key', (name, matches, data) => {
      ...
      switch (eventName) {
        ...
        case 'RIGHT':
          ...
          if (this._selectedItemObj.col < this._columnsCount - 1) this._selectedItemObj.col += 1;
          break;
        case 'LEFT':
          if (this._columnsCount <= 1) return;
      ...
      if (eventName === 'UP' || eventName === 'DOWN' || eventName === 'LEFT' || eventName === 'RIGHT') {
        this.draw();
      }
    });
  }
  ...
  get(settings = {}) {
    ...
    if (this._searchInput.value) items = items.filter(item => item.includes(this._searchInput.value));

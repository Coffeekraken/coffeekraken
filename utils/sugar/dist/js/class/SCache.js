...
class SCache {
  ...
  get(id) {
    ...
    if (!cacheObj) return; // check the lifetime

    if (this.now > cacheObj.expire) {
      // delete the cache item
      delete this.cache[id]; // save the cache

      this._save(); // we not have any cache left
    ...
    return (0, _autoCast.default)(cacheObj.value);
  }
  /**
   * @name 	now
  ...
  set(id, value, lifetime = null) {
    let cacheObj = {}; // create the cache object that need to be stored
    ...
    this.cache[id] = cacheObj; // save the cache

    this._save(); // return the value to store

...
if (!window.sugar._webComponentsClasses)
  window.sugar._webComponentsClasses = {};
...
const SWebComponentMixin = Mixin(
  superclass =>
    class extends superclass {
        ...
        while (comp) {
          try {
            staticFns = staticFns.concat(
              Object.getOwnPropertyNames(comp).filter(
                prop => typeof comp[prop] === "function"
              )
            );
      ...
      get defaultProps() {
        ...
        // compute
        let props =
          window.sugar._webComponentsClasses[this.componentName].defaultProps;
      ...
      get physicalProps() {
        ...
        let props =
          window.sugar._webComponentsClasses[this.componentName].physicalProps;
        let comp = window.sugar._webComponentsClasses[this.componentName];
        while (comp) {
      ...
      get requiredProps() {
        ...
        let props =
          window.sugar._webComponentsClasses[this.componentName].requiredProps;
        let comp = window.sugar._webComponentsClasses[this.componentName];
        while (comp) {
      ...
      connectedCallback() {
        ...
        const sourceName =
          this.getAttribute("is") || this.tagName.toLowerCase();
        this.componentNameDash = this._componentNameDash = sourceName;
        this.componentName = this._componentName = __upperFirst(
          __camelize(sourceName)
        );
        ...
        const observer = new MutationObserver(mutationList => {
          ...
          mutationList.forEach(mutation => {
            if (mutatedAttributes.indexOf(mutation.attributeName) === -1) {
              this._attributeMutationCallback(
        ...
        this._whenMountDependenciesAreOk().then(() => {
          ...
          if (
            ...
            typeof this.props.mountWhen === "function"
          ) {
            this.props
              .mountWhen()
              .then(() => {
            ...
            this.props.mountWhen &&
            typeof this.props.mountWhen === "string"
          ) {
            ...
            switch (this.props.mountWhen) {
              ...
              case "mouseover":
                this.addEventListener(
                  "mouseover",
                  this._onMouseoverComponentMount.bind(this)
                );
                break;
              case "isVisible":
              case "visible":

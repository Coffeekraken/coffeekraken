...
/**
 * @name 		CodemirrorWebcomponent
 * @extends 	SWebComponent
 * @namespace     codemirror-webcomponent
 * @type      Class
 *
...
class InteractiveDemoWebcomponent extends _SWebComponent.default {
  ...
  componentMount() {
    ...
    const mousetrap = new _mousetrap.default(this); // console.log('mouise', __mousetrap);
    ...
    this._codemirror = new _codemirror.default(this, {
      value: content.trim(),
  ...
  _initAccept() {
    ...
    this._refs.modes.className = `${this._componentNameDash}__modes`; // add options
  ...
  _processCode() {
    const promise = new Promise((resolve, reject) => {
      ...
      if (this.props.compile) {
        ...
        this._refs.run.disabled = true; // launch a compileStart event

        this.dispatchComponentEvent('compileStart'); // add class
    ...
    promise.then(code => {
      ...
      if (this.props.compile) {
        this._refs.run.innerHTML = this.props.compiledLabel; // remove class

        this.removeComponentClass(this, null, 'compiling'); // add success class
      ...
      setTimeout(() => {
        ...
        this._refs.run.classList.remove('success'); // add success class
      ...
      this._refs.run.disabled = false;
      setTimeout(() => {
        this._refs.run.classList.remove('error'); // add success class

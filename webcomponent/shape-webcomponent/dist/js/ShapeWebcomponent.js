 ...
 */
class ShapeWebcomponent extends _SWebComponent.default {
  /**
   * Registered shapes
   * @static
   * @type      Object
  ...
  componentMount() {
    super.componentMount(); // apply the top side if specified
  ...
  _applyShapeAtSide(shape, side) {
    ...
    const ratio = viewBox.split(' ')[2] / viewBox.split(' ')[3]; // create and append the svg code

    this[`_${side}`] = (0, _strToHtml.default)(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="${viewBox}" side="${side}" ratio="${ratio || 4.5}"><path fill="currentColor" fill-opacity="1" d="${ShapeWebcomponent.registeredShapes[shape].shape || shape}"></path></svg>`);
    this.appendChild(this[`_${side}`]);
  }

}

exports.default = ShapeWebcomponent;

_defineProperty(ShapeWebcomponent, "registeredShapes", {
  waves1: {
